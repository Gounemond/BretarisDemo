...
// TODO: Softbody system like Maya, so lattice based (option for voxel) then add effectors like wind, turbelence
#if false
[System.Serializable]
public class VoxMass
{
...
[ExecuteInEditMode]
public class MegaFlowMatchShape : MonoBehaviour
{
	public MegaSoft		softbody;
	...
	public void RunPhysics(float t)
	{
		...
		while ( time > 0.0f )
		{
			softbody.Update(softbody.timeStep);

			time -= softbody.timeStep;
		}

		softbody.MoveMasses(softbody);
	}
	...
	public void VoxelMesh(GameObject obj, float size, bool fill)
	{
		...
		// Build bary coords
		barycoords = new BaryCoord[verts.Length];
	...
	void Display1()
	{
		...
		if ( Fill )
		{
			...
			for ( int z = 0; z < zs; z++ )
			{
				...
				for ( int y = 0; y < ys; y++ )
				{
					Gizmos.color = fillcol;	//col1;	//Color.green;
	...
	void Display()
	{
		...
		for ( int m = 0; m < softbody.masses.Count; m++ )
		{
			Vector3 p = softbody.masses[m].pos;
			Gizmos.DrawCube(p, cellsize * 0.05f);
		}

		col = Color.black;
		// Draw springs
		for ( int s = 0; s < softbody.springs.Count; s++ )
		{
			Vector3 p1 = softbody.masses[softbody.springs[s].p1].pos;
			Vector3 p2 = softbody.masses[softbody.springs[s].p2].pos;

			//float len = softbody.springs[s].len;
			//float w = ((softbody.springs[s].len - softbody.springs[s].restlen) / softbody.springs[s].restlen);// * softbody.springs[s].ks;	//con.springs[s].restlen;
			float w = ((Vector3.Distance(p1, p2) - softbody.springs[s].restlen) / softbody.springs[s].restlen);	// * softbody.springs[s].ks;	//con.springs[s].restlen;

			//w /= 0.1f;

			if ( w >= 0.0f )
			{
				float alpha = Mathf.Clamp01(w / 0.01f);
				col = Color.Lerp(Color.green, Color.red, alpha);
			}
			else
			{
				float alpha = Mathf.Clamp01(-w / 0.01f);
				col = Color.Lerp(Color.green, Color.blue, -alpha);
			}
	...
	public List<VoxMass>	voxmasses = new List<VoxMass>();
	...
	void BuildLattice()
	{
			...
			for ( int y = 0; y < ys; y++ )
			{
				...
				for ( int x = 0; x < xs; x++ )
				{
					...
					if ( IsSolid(x, y, z) > 0 )
					{
						...
						for ( int c = 0; c < 8; c++ )
						{
							Vector3 cp = GetCorner(pos, c) * shrink;
		...
		// Build softbody
		softbody.BuildSoftBody(voxmasses, links);
